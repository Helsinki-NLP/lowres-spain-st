# Adapted from https://github.com/Helsinki-NLP/OpusDistillery/blob/main/pipeline/translate/bestbleu.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
...
sp = spm.SentencePieceProcessor(model_file='models/OPUS-MT+bibles/deu+eng+fra+por+spa-itc/opus+jhubc.trg.spm32k-model')
...
def marian_best_bleu(args,score_function):
    if args.rbmt:
        rbmt = args.rbmt.read().splitlines()
    prev_line = None
    for i, ref_line in enumerate(args.references):
        ...
        if args.debpe:
            texts = [sp.decode(t.split()) for t in texts]
        refs = [r.split() for r in refs]

        if args.rbmt:
            rbmt_line = re.sub(r'\*', '', rbmt[i])
            rbmt_score = [score_function(refs, t.split()) for t in [rbmt_line]][0]
        scores = [score_function(refs, t.split()) for t in texts]
        best_txt = texts[scores.index(max(scores))]
        if args.rbmt:
            if rbmt_score > max(scores):
                best_txt = rbmt_line
                print("rbmt")
            else:
                print("nmt")
...
def parse_args():
    ...
    parser.add_argument("--rbmt", default=None, type=FileType('r'), required=False,
                        help="File containing the rule-based translation")
