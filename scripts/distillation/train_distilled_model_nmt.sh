...
for lang in "arg" "ast" "oci" "ara"; do
    ${MARIAN}/spm_encode --model $opusmodeldir/$spm_target ${distilldir}/train.out.nmt.spa-${lang}.${lang} > ${distilldir}/train.out.nmt.nmt.spa-${lang}.${lang}.tok
done
...
cat ${distilldir}/train.out.nmt.spa-arg.arg.tok ${distilldir}/train.out.nmt.spa-ast.ast.tok ${distilldir}/train.out.nmt.spa-oci.oci.tok ${distilldir}/train.out.nmt.spa-ara.ara.tok > ${distilldir}/train.out.nmt.trg.tok

# Use >>ara<< tag for Aranese
#sed 's/^>>xxx<</>>ara<</' "${datadir}/train.src.xxx.ara.tok" > "${datadir}/train.src.ara.ara.tok"
#sed 's/^>>xxx<</>>ara<</' "${datadir}/dev.src.xxx.ara.tok" > "${datadir}/dev.src.ara.ara.tok"

# Fine-tune
${MARIAN}/marian --model "${modeldir}/model.npz" \
       -v "${opusmodeldir}/$vocab" "${opusmodeldir}/$vocab" \
       --train-sets "${datadir}/train.src.ara.ara.tok" ${distilldir}/train.out.nmt.trg.tok \
       --valid-sets "${datadir}/dev.src.ara.ara.tok" "${datadir}/dev.trg.tok"  \
