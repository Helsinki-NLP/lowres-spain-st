#!/bin/bash
#SBATCH --job-name="ensemble_%j.sh"
...
extract_checkpoints() {
  ...
  if [ "$lang" == "ara" ] && [[ "$model_dir" == *xxx* ]]; then
    return
  fi
...
avail_models="models/opus+bibles_ft_all_2_bleu models/opus+bibles_ft_no_oci_bleu"
...
hyp="data/ensembles_beam_1/$NUM_TOP.$LANG"
metric_file="data/ensembles_beam_1/$LANG.metric"

if [ -e $hyp ]; then
    echo "Translation file already exists. Remove to overwrite."
else 
    ${MARIAN}/marian-decoder -m $models -i $src --normalize 1 --beam-size 5 -v $vocab $vocab | ${MARIAN}/spm_decode --model $spm_target > $hyp
fi
...
echo $SLURM_JOB_ID >> data/ensembles_beam_1/jobids.txt